...
class SeqModel(nn.Module):
    ...
    def forward(self, input_src, input_tgt, srcmask, srclens, input_attr,
                attrlens, attrmask, side_info):
        src_emb = self.src_embedding(input_src)

        srcmask = (1 - srcmask).bool()
        ...
        # # # #  # # # #  # #  # # # # # # #  # # seq2seq diff
        if self.config['experimental']['predict_sides']:
            side_info = side_info[:, 1:].squeeze(
                1)  # ignore the "start token" from data.get_minibatch
            query = torch.zeros(src_outputs[:, 0, :].shape)
            if CUDA:
                query = query.cuda()
            src_summary, _, probs = self.side_attn(query=query,
                                                   keys=src_outputs,
                                                   values=src_outputs,
                                                   mask=srcmask)

            side_logit, side_loss = self.side_predictor(src_summary, side_info)
            if self.config['experimental']['add_side_embeddings']:
        ...
        tgt_outputs, (_, _) = self.decoder(tgt_emb, (h_t, c_t), src_outputs,
                                           srcmask)
        ...
        decoder_logit = decoder_logit.view(tgt_outputs.size()[0],
                                           tgt_outputs.size()[1],
...
class TextClassifier(object):
    ...
    def parameters(self):
        ...
        out = {
            id2tok[i]: coef
            for i, coef in enumerate(self.predictor.coef_[0])
        }
        return out

    def fit(self, corpus1_path, corpus2_path, seed=0):
        sents = [x.strip() for x in open(corpus1_path)
                 ] + [x.strip() for x in open(corpus2_path)]
    ...
    def error_rate(self, seqs, Y):
        ...
        error = len([yhat
                     for yhat, y in zip(Y_hat, Y) if yhat != y]) * 1.0 / len(Y)
        return error
